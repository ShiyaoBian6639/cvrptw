...
objective = xsum(dist_mat[(i, j)] * x[(i, j, k)] for i in N for j in N for k in V)
...
for k in range(num_vehicle):
    m.add_constr(xsum(x[(0, j, k)] for j in N) == 1, name=f"start_{k}")

# each vehicle ends at the depot
for k in range(num_vehicle):
    m.add_constr(xsum(x[(i, n + 1, k)] for i in N) == 1, name=f"end_{k}")
#
# arc balancing
for h in C:
    for k in range(num_vehicle):
